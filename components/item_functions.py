...
def cast_telekinesis(*args, **kwargs):
    entities = kwargs.get('entities')
    fov_map = kwargs.get('fov_map')
    damage = kwargs.get('damage')
    ...
    results.append({'consumed': True, 'message': Message('You damaged everyone within {0} tiles!'.format(radius), libtcod.orange)})

    for entity in entities:
        if entity.distance(target_x, target_y) <= radius and entity.fighter:
            results.append({'message': Message('The {0} gets dropped on the floor for {1} hit points.'.format(entity.name, damage), libtcod.orange)})
